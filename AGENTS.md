...
| 訊號類別 | 範例語句 |
|---|---|
| 新增功能 | 「加一個 X」「實作 ...」「做一個 ...」「製作 ...」「寫一個 ...」「I want to add ...」「build a ...」「make a ...」 |
| 修改既有行為 | 「把 X 改成 Y」「重構 ...」「升級 ...」「migrate ...」 |
| 技術選型 | 「選什麼 X」「決定用 ...」「評估 A vs B」「pick a framework」 |
| 設計題目 | 「設計 ...」「規劃 ...」「architecture for ...」 |

**規則沒有例外**: 任務的**大小、複雜度、嚴肅性**不是觸發條件 — **任務類型才是**。
小如 toy / demo / POC / 學習練習 / 一頁遊戲, 大如完整產品功能, **只要符合上面 4 類訊號之一,一律啟動 SDD**。
...
- ❌ 跳過 spec 直接動 code, 含各種變體 (「先實作簡單版」/「prototype 試水溫」/「事後補文件化」) — 永遠不會補
- ❌ 反問「要不要走 SDD?」/「要不要叫 Codex?」 — 預設要, 例外條件見 §3 與下方「不觸發的場合」
- ❌ **「這太簡單, 直接寫吧」/「toy / demo / POC 走流程很重」** — 任務大小不是判斷依據, 任務類型才是
- ❌ 自行猜「使用者大概不需要這麼正式」 — 預設要走, 例外才在 audit trail 寫具體理由

**不觸發的場合** (純粹 patch / 答覆即可, 是**任務類型**不適用 SDD, 不是「任務太簡單」可繞):

- 純 bugfix (改幾行、有對應現有 spec scenario)
- 純 rename / 樣式調整 / 文件更新
- 問答 / 解釋 / 程式碼閱讀
- 使用者明確說「不要走 SDD, 我只要 X」(禮貌確認後跳過)
