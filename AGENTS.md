...
- ❌ **「這太簡單, 直接寫吧」「toy 不需要 spec」「demo 走流程很重」** — 任務複雜度不是判斷依據
- ❌ 自行判斷「使用者大概不需要這麼正式」 — 預設要走, 例外才在 audit trail 寫理由
- ❌ 寫一個 prototype 試水溫, 之後「再補 SDD 文件」 — 永遠不會補
...
**注意**: 上述例外都是「**任務類型**不需要 spec」, 不是「任務太簡單」。如果你在判斷時想用「太簡單」當理由, 那是 §0 反模式, 請改回啟動 SDD。
